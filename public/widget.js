...
(function () {
  function mount() {
    // Find the placeholder element on the Shopify product page
    var host =
    ...
    // Base URL of your Vercel app (this script is served from there)
    var baseUrl = new URL(document.currentScript.src).origin;

    // Embed your public widget route in an iframe
    ...
    // Listen for widget -> Shopify page messages
    window.addEventListener('message', function (event) {
      try {
        ...
        if (data.type === 'h2h:reservation:ready') {
          // Unlock Dawn Add to Cart (your buy-buttons.liquid listens for this)
          window.dispatchEvent(new CustomEvent('h2h:reservation:ready'));

          // Also store jersey number into hidden line-item property input if present
          var input = document.getElementById('h2h_jersey_number');
          if (input) input.value = String(data.jerseyNumber);

          // Optional: trigger input listeners
          if (input) {
