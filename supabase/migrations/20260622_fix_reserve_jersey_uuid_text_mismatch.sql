-- Discovered during Task #32/#35 testing: pl.club_id is uuid, but the function
-- compared it against p_club_id::text, causing "operator does not exist: uuid = text"
-- whenever a returning player's first/last name was supplied. This silently broke
-- the team-gender-aware Mixed-pool detection for ALL returning-player reservations.
